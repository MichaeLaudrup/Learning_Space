    ...
    if(pidResultanteFork > 0){ //Codigo del padre
        ...
        do{
            ...
            for (int i = 0; i < num_procVivos; i++){
                ...
                usleep(1); //El tiempo entre mensaje y mensaje es de 1 ns
            }
            int mensajesPendientes = num_procVivos;
            usleep(400000); //Se esperan 3 milsegundos a que acabe la contienda actual
        ...
        //printf("Mensaje que se debe almacena en la tuberia FIFO: %s", mensaje1); //Depurando el mensaje esta bien // SOLO PARA DEPURAR
        ...
        if( close(descriptor_tuberia) == -1){perror("Error en el ciere de la tuberia: ");}

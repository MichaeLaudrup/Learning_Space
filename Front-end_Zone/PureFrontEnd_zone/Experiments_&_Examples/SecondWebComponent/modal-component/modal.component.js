class MyModalHTMLElement extends HTMLElement {
    ...
    _requestHTMLCssFiles() {
        this._getJSONData(this.cssURL, true);
        this._getJSONData(this.htmlURL, false);
    }

    _getJSONData(fileURL, isStyleURL) {
    ...
    _renderHTML() {
            ...
            <div class="dialog">
                ...
                <footer class="dialog-foot">
                    ${this.textButtons.map((textButton,index) => {
                        return `<button class="btn-modal" onclick="${this.jsCodeButtons[index]}">${textButton}</button>`
                    }).join('')}
                </footer>
        ...
        this.shadow.querySelector('.close').onclick = function (){
            shadow.innerHTML = ""; 
        }
    }
